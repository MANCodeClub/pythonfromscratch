#pip install lxml #pour disposer des fonctions xml utilisées par la suite
#pip install pathlib #pour tenter de lire tout le contenu d'un répertoire!
# MN dictionnaire imbriqué
...
export_FEN = open("export_FEN.txt","w",encoding='utf-8')#j'ouvre en écriture un fichier pour récupérer les titres avec le nom DDT comme redevable
...
def apure(file):
    n = 0
    FEN_Source = open(file,"r",encoding="ISO-8859-15")
    FEN_XML = open(file+".xml","w",encoding="utf-8")
...
def parse(fic):
    ...
    while o < len((TX_Final)[0]):
        for designation in TX_Final[1][o].keys():
            #print (designation)
            if (designation.upper().find('DDT')) != -1 :
...
"""
for fichier in os.listdir("C:\\_Mes_Etudes\\MCO06_Exploitation\\2016_FEN\\A_Virer\\INPUT"):
    apure(fichier)
"""

for fichier in os.listdir("C:\\_Mes_Etudes\\MCO06_Exploitation\\2016_FEN\\A_Virer\\OUTPUT"):
    parse("C:\\_Mes_Etudes\\MCO06_Exploitation\\2016_FEN\\A_Virer\\OUTPUT\\"+fichier)
