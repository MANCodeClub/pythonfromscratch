...
#token = requests.post(url = 'https://api.insee.fr/token', headers = headers, data=data, verify=False)
...
FILE_PATH="J:/SI/SI ADS/MCO/MCO6 Exploitation/2021_SH_MN_SIRET"

#print(f"{FILE_PATH}/SIRET_A_Controler.txt")

fichier_RESULTAT = open(f"{FILE_PATH}/fichier_RESULTAT.txt","w",encoding='utf-8')
...
def Verif(fichier):
    ...
    for row in lecteur :
        #print (row)
        dict_com_input['commune_info'].append({'code_insee':row[0],'libelle_commune':row[1],'siret_communal':row[2],'siret_epci':row[3]})
    #print (dict_com_iput)

    for SIRET_Com in (dict_com_input['commune_info']) : # Pour chaque SIRET du fichier,
        #print (({SIRET_Com['siret_communal']}))
        if "" in ({SIRET_Com['siret_communal']}):
            print ("vide")
            fichier_RESULTAT = open(f"{FILE_PATH}/fichier_RESULTAT.txt","a",encoding='utf-8')
            ...
            fichier_RESULTAT.write(f"SIRET_MANQUANT_COTE_ADS2007\n")
        else : 
            fichier_RESULTAT = open(f"{FILE_PATH}/fichier_RESULTAT.txt","a",encoding='utf-8')
